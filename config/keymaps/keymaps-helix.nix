# keymaps-helix.nix
# Helix-inspired keybindings for Neovim
{
  config.keymaps = [
    # --- 1. Find/Till Commands ---
    # In Helix, f, F, t, T are not confined to the current line
    {
      mode = "n";
      key = "f";
      action.__raw = ''
    ...
    }
    {
      ...
      action.__raw = ''
        function()
          ...
          -- Search in current line first, starting from current position backward
          local pos = vim.fn.search(vim.fn.escape(char, '[]\\^$.*'), 'b', line)
          if pos > 0 and vim.fn.col('.') < col then
            found = true
          else
            -- If not found, search in previous lines
            vim.cmd('normal! ' .. line .. 'G' .. col .. '|')
            pos = vim.fn.search(vim.fn.escape(char, '[]\\^$.*'), 'b')
            if pos > 0 then
              found = true
            end
          end
      ...
      '';
      options = { noremap = true; silent = true; desc = "Helix: Find previous char (not line-confined)"; };
    }
    {
      mode = "n";
      key = "t";
      action.__raw = ''
        function()
          ...
          local pos = vim.fn.search(vim.fn.escape(char, '[]\\^$.*'), '', line)
          if pos > 0 and vim.fn.col('.') > col then
            -- Move one character back for 'till'
            vim.cmd('normal! h')
            found = true
          else
            -- If not found, search in subsequent lines
      ...
      '';
      options = { noremap = true; silent = true; desc = "Helix: Find till next char (not line-confined)"; };
    }
    {
      mode = "n";
      key = "T";
      action.__raw = ''
        function()
          ...
          local pos = vim.fn.search(vim.fn.escape(char, '[]\\^$.*'), 'b', line)
          if pos > 0 and vim.fn.col('.') < col then
            -- Move one character forward for 'till'
            vim.cmd('normal! l')
            found = true
          else
            -- If not found, search in previous lines
      ...
      '';
      options = { noremap = true; silent = true; desc = "Helix: Find till previous char (not line-confined)"; };
    }
    {
      mode = "n";
      key = "<A-.>";
